...
export default function ScrollTo({ coord }: { coord: string }) {
  return (
    <div className="flex items-center justify-center mx-auto gap-20 my-32">
      {scrollLinks.map((item, i) => {
        return (
          <button
            className={`w-[10px] h-[10px] border-spacing-[2px] border-solid cursor-pointer hover:bg-white duration-[.2s] ${
              coord === item && "bg-white"
            }`}
            key={i}
