...
기존에 구성된 UI 구조를 기반으로 Zustand Store를 도입하여 로그인 여부 및 사용자 권한에 따른 조건 분기 로직을 추가하였고, hydrate 완료 여부를 조건부 렌더링에 반영함으로써 초기 로딩 시 UI 깜빡임을 최소화했습니다.`,
          ...
          },
          {
            ...
            description: `모달은 공통 컴포넌트와 기능 전용 모달로 구성되어 있으며, 다양한 상황에서 유연하게 재사용될 수 있도록 설계했습니다.
공통 모달 컴포넌트는 $기능명 프롭스를 통해 UI와 동작을 유동적으로 구성할 수 있고, modalController 커스텀 훅을 통해 모달의 열림/닫힘 상태와 메시지를 개발자가 직접 제어할 수 있도록 했습니다.
이 모달은 삭제 경고나 알림 등 사용자에게 피드백을 제공해야 하는 다양한 상황에 활용됩니다. 반면, ‘내 정보 수정’이나 ‘비밀번호 변경’과 같은 페이지 전용 모달은 각 디렉토리 내부에서 독립적으로 관리되며, 재사용성과 유지보수성을 고려한 구조로 구현했습니다.`,
            image:
...
기존의 반응형 레이아웃은 유지하면서도, 과도하게 확장되던 메인 섹션의 시각적 불안정성을 개선했으며, 사용자 상태에 따라 CTA 버튼의 이동 경로를 유동적으로 설정해 보다 매끄럽고 개인화된 온보딩 경험을 제공합니다.`,
          ...
          },
          {
            ...
            contributionNote: false,
            description: `사용자의 서비스 진입을 위한 첫 단계로, 회원가입과 로그인, 회원 정보 입력을 순차적으로 처리하는 구조를 갖추고 있습니다. 회원가입 이후에는 사용자 유형(채용자/지원자)에 따라 추가 정보를 입력받도록 분기하며, 로그인 과정에서는 액세스/리프레시 토큰을 활용한 인증 체계를 적용해 보안성을 높였습니다.
또한 카카오 소셜 로그인을 포함한 다양한 접근 방식을 제공함으로써 접근성을 강화했고, Zustand를 기반으로 사용자 흐름을 일관되게 유지할 수 있도록 구성했습니다.`,
            image:
...
초기 렌더링 속도를 개선하기 위해 SSR과 CSR 방식을 통합하여 적용했으며, 검색, 필터, 무한스크롤 기능을 통해 다양한 조건에 맞는 공고 탐색을 지원합니다. 또한 모바일 환경에서도 최적화된 UI를 제공해, 기기와 관계없이 모든 사용자에게 원활한 탐색 경험을 보장합니다.`,
          ...
          },
          {
            ...
            contributionNote: false,
            description: `채용자가 새로운 알바 공고를 작성하거나 기존 공고를 수정할 수 있는 입력 폼 페이지로, React Hook Form(RHF)을 사용해 입력 데이터를 관리합니다. 모든 필수 항목이 작성되어야만 제출할 수 있도록 유효성 검사를 포함했고, 폼 상태 변화에 따라 상위 컴포넌트로 데이터를 전달하며, 최종 제출 시에는 서버로 안전하게 전송되도록 구현했습니다.`,
            image:
          ...
          },
          {
            ...
            contributionNote: true,
            description: `알바폼 상세 페이지는 채용자와 지원자에게 각기 다른 인터페이스를 제공하며, 공고의 핵심 정보를 시각적으로 명확하게 전달하는 역할을 합니다.
초기에는 정적인 텍스트 기반 구조였으나, 지도 연동, 공유 기능, 액션 버튼, 반응형 스타일링 등 다양한 기능을 추가해 실사용에 적합한 상세 페이지로 발전시켰습니다.
채용자는 페이지 하단에서 지원자 목록을 무한스크롤로 확인할 수 있고, 지원자는 스크랩 기능을 통해 관심 공고를 손쉽게 저장할 수 있도록 구성했습니다.`,
            image:
...
React Hook Form과 Zod 기반의 스키마 유효성 검사를 활용해 입력 흐름을 자연스럽게 제어하며, 불완전한 제출을 방지하도록 설계했습니다. 또한 백엔드에서 임시 저장 기능을 지원하지 않는 한계를 보완하기 위해 로컬스토리지를 활용한 임시 저장 기능을 구현하여 사용자 경험을 향상시켰습니다.`,
          ...
          },
          {
            ...
            contributionPercent: 100,
            contributionNote: true,
            description: `지원내역 상세 페이지는 채용자와 지원자가 각자의 입장에서 지원 정보를 확인하고 필요한 조치를 취할 수 있도록 설계되었습니다. 지원자는 자신이 제출한 지원 내용과 자기소개, 첨부한 이력서를 확인할 수 있으며, 채용자는 지원자의 상세 정보를 조회하고 지원 상태를 변경할 수 있습니다. 지원 상태 변경은 공통 모달 컴포넌트를 활용해 ‘거절’, ‘면접 대기’, ‘면접 완료’, ‘채용 완료’ 등의 상태로 유연하게 처리됩니다.`,
            image:
          ...
          },
          {
            ...
            contributionPercent: 100,
            contributionNote: false,
            description: `알바토크는 사용자 간 자유로운 소통을 위한 커뮤니티 게시판 기능으로, 다양한 주제의 글을 자유롭게 탐색하고 확인할 수 있는 공간입니다. 게시글 목록은 IntersectionObserver를 활용한 무한스크롤 방식으로 구현되어, 사용자가 자연스럽게 콘텐츠를 이어서 탐색할 수 있습니다. 또한 검색 기능과 정렬 필터를 함께 제공해 원하는 게시글을 효율적으로 찾을 수 있도록 구성했습니다.`,
            image:
          ...
          },
          {
            ...
            contributionPercent: 100,
            contributionNote: false,
            description: `사용자는 이 페이지에서 자유롭게 게시글을 작성하거나 기존 글을 수정할 수 있습니다. 
'등록'과 '수정'은 동일한 UI 구조를 유지하고, 변경사항이 없거나 필수 입력 조건이 충족되지 않을 경우 등록 버튼이 비활성화되어 불완전한 제출을 방지하도록 설계했습니다.
또한, 이미지 첨부는 한 게시글당 최대 1개로 제한하여 사용자 경험을 단순하고 명확하게 유지했습니다.`,
            image:
          ...
          },
          {
            ...
            contributionPercent: 100,
            contributionNote: false,
            description: `사용자가 자유게시판 형태의 게시글을 열람할 수 있는 페이지로, 이미지가 첨부된 경우 콘텐츠는 이미지와 텍스트 순서로 구성됩니다. 다양한 이미지 비율에 대응하기 위해 useEffect를 활용해 동적으로 크기를 계산하여 렌더링하며, 댓글은 유틸리티 함수를 통한 페이지네이션과 React Query 기반 데이터 처리로 관리되어 수정 및 삭제 시 실시간으로 반영됩니다.
또한 좋아요 기능은 옵티미스틱 UI로 구현되어 빠른 인터랙션 피드백을 제공합니다.`,
            image:
          ...
          },
          {
            ...
            contributionPercent: 100,
            contributionNote: false,
            description: `개인화된 알바폼 목록 페이지로, 사용자 권한에 따라 다른 정보를 제공합니다.
채용자는 자신이 작성한 공고를 확인할 수 있고, 지원자는 자신이 지원한 알바 목록을 열람할 수 있도록 구성했습니다. 검색과 정렬 필터 기능을 통해 원하는 항목을 빠르게 찾을 수 있으며, IntersectionObserver를 활용한 무한 스크롤 방식으로 부드러운 사용자 경험을 유지합니다.`,
            image:
        ...
        features: [
          {
            ...
            contributionPercent: 100,
            contributionNote: false,
            description: `첫 진입 시 사용자에게 브랜드 인상을 전달하는 랜딩 페이지로, PC, 태블릿, 모바일 환경에서 모두 자연스럽게 작동하는 반응형 UI로 구현했습니다.
메인 배너의 ‘구경하러 가기’ 버튼 클릭 시 로그인 여부를 판단하여, 로그인 상태일 경우 중고마켓 목록으로, 비로그인 상태일 경우 로그인 페이지로 이동하도록 분기 처리를 적용했습니다.`,
            image:
          ...
          },
          {
            ...
            contributionPercent: 100,
            contributionNote: false,
            description: `서비스 접속의 첫 단계로 로그인과 회원가입 기능을 제공하는 페이지입니다.
React Context API와 Provider 패턴을 활용해 입력 상태를 전역으로 관리함으로써 컴포넌트 간 일관성을 유지했습니다. 입력 값에 대한 유효성 검사는 하드코딩으로 직접 구현했고, React Query를 사용해 서버와 안정적으로 데이터를 주고받도록 처리했습니다.
현재 SNS 로그인 기능은 개발이 중단된 상태이며, 추후 포트폴리오 완성 후 추가 개발할 계획입니다.`,
          ...
          },
          {
            ...
            contributionPercent: 100,
            contributionNote: false,
            description: `중고마켓 사용자들이 다양한 상품을 편리하게 탐색하고 구매할 수 있도록 설계된 핵심 페이지입니다. 사용자는 키워드 검색 기능을 통해 원하는 상품을 즉시 찾을 수 있으며, 검색어 입력과 동시에 결과가 실시간으로 필터링되어 높은 반응성을 경험할 수 있습니다. 또한 최신순과 좋아요순 정렬 기능을 제공해 사용자가 선호하는 기준에 따라 상품을 쉽게 정렬할 수 있도록 지원합니다.
베스트 상품과 전체 상품을 구분하여 보여주며, 반응형 디자인에 맞춰 한 화면에 표시되는 상품 수를 자동으로 조절해 기기별로 최적화된 사용자 경험을 제공합니다.`,
            image:
        ...
        features: [
          {
            ...
            contributionPercent: 0,
            contributionNote: false,
            description: `사용자가 전체 체험 콘텐츠를 탐색할 수 있도록 인기 체험과 전체 체험 섹션으로 구성된 메인 페이지입니다.
인기 체험은 slick-slider를 활용한 슬라이드 UI로 제공되며, 전체 체험 목록은 숫자 기반 페이지네이션 방식을 적용해 효율적인 데이터 로딩이 가능합니다.
또한 제목 기반 키워드 검색과 다양한 정렬 조건을 제공하여 사용자가 원하는 체험을 빠르게 찾을 수 있도록 지원합니다.`,
            image:
          ...
          },
          {
            ...
            contributionPercent: 0,
            contributionNote: false,
            description: `사용자의 계정 인증을 위한 로그인 및 회원가입 기능을 React Hook Form과 Zod를 조합해 각 입력 항목에 대해 정교한 유효성 검사를 적용하여 구현했습니다.
서버와의 데이터 송수신은 React Query로 처리하며, 비동기 상태에 따른 로딩과 에러 대응이 안정적으로 이루어집니다.
또한 카카오 로그인 연동을 통해 간편한 소셜 로그인 기능을 제공하고, 신규 회원은 카카오 정보를 기반으로 자동 회원가입이 가능합니다. 가입 및 로그인 완료 후에는 react-hot-toast를 활용해 사용자에게 직관적인 결과 메시지를 안내합니다.`,
            image:
          ...
          },
          {
            ...
            contributionPercent: 0,
            contributionNote: false,
            description: `체험 상품에 대한 상세 정보를 제공하며, 카카오 지도를 연동해 사용자가 체험 장소를 시각적으로 확인할 수 있도록 구현했습니다.
예약 가능한 날짜, 시간, 인원 수를 선택해 예약 신청이 가능하며, 후기는 숫자 기반 페이지네이션 방식으로 분리하여 확인할 수 있습니다. 다만, 필수 항목이 선택되지 않은 경우 버튼 비활성화 처리 등 UX 측면에서의 유효성 피드백은 다소 미흡하여 추후 개선이 필요합니다.`,
            image:
          ...
          },
          {
            ...
            contributionPercent: 0,
            contributionNote: false,
            description: `사용자가 닉네임, 프로필 이미지, 비밀번호를 수정할 수 있는 개인 정보 관리 페이지로, 모달 UI를 통해 입력 정보를 손쉽게 변경할 수 있도록 구성되었습니다. 사용자 경험을 고려한 직관적이고 편리한 인터페이스로 구현되었습니다.`,
            image:
          ...
          },
          {
            ...
            contributionPercent: 100,
            contributionNote: false,
            description: `사용자가 참여한 체험의 예약 상태를 한눈에 확인하고 필요한 조치를 취할 수 있는 예약 관리 페이지로, 스크롤 위치를 유지하며 데이터를 무한스크롤 방식으로 불러오기 위해 직접 구현한 커스텀 훅(useScrollPositioning)을 활용했습니다.
예약 상태에 따른 정렬 필터, 예약 취소, 후기 작성 기능을 제공하며, 후기는 별점 드래그 인터랙션이 적용된 모달을 통해 작성할 수 있습니다. 또한, 기획에는 없던 기능으로 예약 기한 만료나 후기 작성 완료 여부를 판단해 상황에 맞는 안내 문구를 동적으로 표시하는 로직을 추가로 개발했습니다.`,
            image:
          ...
          },
          {
            ...
            contributionPercent: 0,
            contributionNote: false,
            description: `커스텀 훅(useScrollPositioning)을 활용해 ‘내가 등록한 체험’ 목록을 무한스크롤 방식으로 불러올 수 있도록 구현했습니다. 페이지 상단에는 새로운 체험 등록으로 이동할 수 있는 버튼을 배치했으며, 각 체험 카드에는 ‘더보기’ 버튼을 통해 수정 및 삭제 작업이 가능하도록 구성했습니다.
사용자 입장에서 체험 목록의 탐색, 관리, 등록 흐름이 자연스럽게 이어지도록 인터페이스를 설계했습니다.`,
            image:
          ...
          },
          {
            ...
            contributionPercent: 0,
            contributionNote: false,
            description: `내 체험을 등록하거나 수정할 수 있는 공통 UI 폼 페이지로, 입력값을 직접 상태로 관리하여 payload를 구성한 후 React Query를 활용해 서버로 전송하는 방식으로 구현되었습니다.
카카오 주소 검색 기능이 연동되어 위치 정보를 효율적으로 입력할 수 있고, 배너 이미지는 1개, 서브 이미지는 최대 4개까지 업로드가 가능합니다. 다만, 현재 필수 입력값 검증과 버튼 활성화 조건이 분리되어 있어 UX 측면에서 다소 아쉬움이 있으며, 추후 조건부 활성화 로직 개선이 필요합니다.`,
            image:
          ...
          },
          {
            ...
            contributionPercent: 0,
            contributionNote: false,
            description: `react-calendar 라이브러리를 활용해 달력 UI를 구현하였으며, 사용자가 직접 등록한 체험을 선택하면 해당 체험의 예약 현황을 달력을 통해 한눈에 확인할 수 있습니다. 또한, 달력 내에서 예약 승인과 거절 기능을 제공해 예약 관리 업무를 보다 직관적이고 효율적으로 수행할 수 있도록 설계했습니다.`,
            image:
...
또한, 클릭 이벤트 핸들러와 조건부 렌더링을 활용해 사용자 상호작용과 내비게이션 용도로 유연하게 활용할 수 있도록 구현되었습니다.`,
          ...
          },
          {
            ...
            contributionPercent: 0,
            contributionNote: false,
            description: `React Portal을 활용해 모달 콘텐츠를 DOM 최상단에 렌더링하며, 뒷배경 클릭 시 모달이 닫히도록 인터랙션을 제공합니다. 또한, 모달 외부 영역 클릭 시 이벤트 전파를 차단하여 의도치 않은 닫힘을 방지하고, 다양한 콘텐츠에 대응할 수 있도록 children 기반의 확장형 구조로 설계되었습니다.`,
            image:
          ...
          },
          {
            ...
            contributionNote: false,
            description: `사용자 로그인 정보를 기반으로 대시보드 이름, 관리 기능, 멤버 목록, 프로필 드롭다운을 표시하는 글로벌 내비게이션 컴포넌트입니다.
        ...
        features: [
          {
            ...
            contributionPercent: 0,
            contributionNote: false,
            description: `서비스의 첫 인상을 담당하는 랜딩 페이지로, 정적인 시각 요소에 AOS(Animate on Scroll)를 적용해 부드러운 인터랙션 효과를 구현했습니다.
React Type Animation을 활용하여 제품명 ‘Taskify’가 반복적으로 타이핑되는 애니메이션을 연출함으로써 브랜드 이미지를 강화했습니다. 또한 CustomButton 컴포넌트와 함께 ‘로그인하기’ 버튼을 배치하여 사용자 흐름의 시작 지점을 명확하게 유도합니다.
초기에는 다크모드 테마를 고려해 흰 배경으로 시작했으나, 실제 개발 과정에서는 테마 기능이 제외되어 현재는 라이트 모드만 제공되고 있습니다.`,
            image:
          ...
          },
          {
            ...
            contributionPercent: 0,
            contributionNote: false,
            description: `회원가입과 로그인은 유사한 구조로 설계되어 있으며, 이메일, 닉네임, 비밀번호 입력에 대해 정적인 유효성 검사를 직접 구현하여 실시간 피드백을 제공합니다. 입력값 검증은 별도의 라이브러리 없이 정규표현식과 조건문을 기반으로 처리되며, 이용약관 동의와 같은 필수 항목이 모두 완료되어야만 가입 버튼이 활성화됩니다.
비밀번호와 비밀번호 확인 입력창에는 눈 아이콘을 활용한 가시성 토글 기능을 포함해 사용자의 편의성을 높였으며, 가입 또는 로그인 성공 및 실패 시에는 CustomModal을 통해 직관적인 결과 피드백을 제공합니다. 또한 회원가입 완료 후에는 자동으로 로그인 페이지로 리디렉션되어 자연스러운 사용자 흐름을 유도합니다.`,
          ...
          },
          {
            ...
            contributionPercent: 0,
            contributionNote: false,
            description: `대시보드 개요 페이지는 사용자가 자신이 생성한 대시보드, 소속된 대시보드, 그리고 초대받은 대시보드를 한눈에 관리할 수 있도록 구성되어 있습니다.
초대받은 대시보드는 수락 여부에 따라 별도로 분리되어 표시되며, 각 영역은 디자인과 목적에 따라 무한스크롤 또는 페이지네이션 방식으로 구현되어 있습니다. 사용자는 ‘새로운 대시보드 생성’ 기능을 통해 이 페이지에서 바로 협업 공간을 추가로 만들 수 있으며, 각 대시보드 카드를 클릭하면 해당 대시보드 상세 페이지로 진입하여 여러 협업 공간을 유연하게 전환할 수 있습니다.
데이터 통신은 axios와 useEffect를 조합해 구현하였으며, 직접적인 비동기 처리와 상태 관리를 담당합니다.`,
            image:
          ...
          },
          {
            ...
            contributionPercent: 0,
            contributionNote: false,
            description: `‘나의 대시보드’ 페이지에서 접근할 수 있는 대시보드 생성 전용 모달 컴포넌트는, 사용자가 입력한 대시보드 이름과 색상 정보를 axios를 이용해 서버에 POST 요청으로 전송하여 새로운 대시보드를 생성합니다.
대시보드 이름에 대해서는 빈 값 입력을 방지하기 위한 유효성 검사를 진행하며, 색상 선택은 미리 정의된 리스트 중 하나를 클릭하여 지정할 수 있습니다.
색상이 선택되면 체크 아이콘이 표시되고, 이름과 색상이 모두 유효할 경우 ‘생성’ 버튼을 눌러 대시보드를 실제로 생성하며, 생성된 정보는 부모 컴포넌트에 반영됩니다. 입력과 상태 관리는 모두 이 컴포넌트 내부에서 처리되며, 별도의 상태관리 라이브러리 없이도 독립적인 데이터 흐름이 완성된 구조입니다.`,
            image:
          ...
          },
          {
            ...
            contributionPercent: 100,
            contributionNote: false,
            description: `이 페이지는 사용자가 대시보드의 핵심 정보를 관리하고 구성원을 효율적으로 운영할 수 있도록 설계된 관리 페이지입니다. 현재 소속된 구성원과 초대 내역을 확인할 수 있으며, 구성원 삭제 및 초대 취소 기능을 통해 팀 관리를 원활하게 수행할 수 있습니다.
또한, 통합된 초대 기능을 제공하여 신규 구성원 초대가 쉽게 이루어집니다. 모든 변경 사항은 즉각적으로 화면에 반영되어 사용자 경험의 연속성을 보장합니다.
마지막으로, ‘대시보드 삭제’ 기능을 통해 불필요한 대시보드를 손쉽게 제거할 수 있으며, 삭제가 완료되면 자동으로 메인 대시보드 목록으로 이동하여 자연스러운 작업 흐름을 유지합니다.`,
            image:
          ...
          },
          {
            ...
            contributionPercent: 0,
            contributionNote: false,
            description: `이 페이지는 특정 대시보드에 속한 컬럼과 해당 컬럼 내 메모들을 확인하고 관리할 수 있도록 설계되었습니다. 대시보드 ID를 기준으로 API를 호출해 해당 대시보드의 컬럼들을 불러오며, 각 컬럼 내부에는 텍스트와 이미지 기반의 메모가 시각적으로 표시됩니다.
컬럼들은 독립적인 컴포넌트로 구성되어 있고, 사용자는 전용 모달을 통해 컬럼명을 입력하여 새로운 컬럼을 생성할 수 있습니다. 컬럼 삭제 기능도 지원하며, 삭제 시 UI에 즉시 반영됩니다.
드래그 앤 드롭 기능을 위해 @hello-pangea/dnd 라이브러리가 적용되어 있으나, 실제 드래그 이동 기능은 구현되지 않은 상태로 마무리되었습니다. API 호출과 데이터 흐름 관리는 axios와 useEffect 조합으로 처리하고 있습니다.`,
            image:
          ...
          },
          {
            ...
            contributionPercent: 0,
            contributionNote: false,
            description: `새로운 칼럼을 생성하고 기존 칼럼명을 변경할 수 있는 기능을 제공하며, 사용자가 입력한 칼럼명을 서버에 POST 요청으로 전송한 후 리스트와 변경 사항이 실시간으로 반영되도록 구현되었습니다.`,
            image:
          ...
          },
          {
            ...
            contributionPercent: 0,
            contributionNote: false,
            description: `할 일 카드의 등록과 수정을 위한 모달 컴포넌트로, 사용자는 제목, 설명, 마감일, 담당자, 태그, 이미지 등을 입력할 수 있습니다.
필수 입력 항목은 제목, 설명, 마감일, 담당자이며, 담당자는 현재 대시보드 멤버 중에서 선택할 수 있고 선택된 담당자는 프로필 이미지나 닉네임의 첫 글자로 표시됩니다. 태그는 Enter 키를 통해 여러 개를 추가할 수 있고 이미지 업로드 기능도 포함되어 있습니다. 마감일은 시간 단위까지 선택 가능한 데이트피커로 설정하며, 입력값 검증 후 서버에 카드 데이터를 전송하는 구조로 구현되어 있습니다.`,
            image:
          ...
          },
          {
            ...
            contributionPercent: 0,
            contributionNote: false,
            description: `카드의 세부 정보를 확인하고 댓글을 관리할 수 있는 모달로, 카드 제목, 설명, 태그, 담당자 정보, 마감일, 첨부 이미지 등 모든 관련 데이터를 직관적으로 확인할 수 있도록 구성되었습니다. 
담당자는 이름 또는 프로필 이미지로 표시되며, 마감일과 함께 레이아웃 우측에 시각적으로 구분됩니다. 댓글 목록은 서버에서 조회되고, 새로운 댓글 작성 시 서버에 전송 후 목록이 실시간으로 갱신되도록 구현되었습니다. 또한 카드 수정 버튼을 통해 별도의 수정 모달(TaskEditModal)을 열 수 있으며, 수정 후 상세 정보가 즉시 반영됩니다.`,
            image:
          ...
          },
          {
            ...
            contributionNote: true,
            description: `계정 관리 페이지는 사용자가 자신의 계정 정보를 관리할 수 있는 공간으로, 프로필 수정과 비밀번호 변경 기능을 제공합니다. 프로필 수정 섹션에서는 사용자가 닉네임과 프로필 이미지를 변경할 수 있으며, 변경 사항은 실시간으로 반영됩니다.
비밀번호 변경 기능은 별도의 입력 검증 로직과 함께 구성되어 보안성과 사용성을 동시에 고려한 설계가 적용되어 있습니다.`,
            image:
        ...
        features: [
          {
            ...
            contributionNote: false,
            description: `서비스의 첫 진입점 역할을 하는 이 페이지는 사용자가 롤링페이퍼 작성 기능에 자연스럽게 접근하도록 유도합니다. 기초적인 서비스 소개와 사용 흐름 안내로 구성되어 있으며, 시각적 집중을 위해 간결한 레이아웃으로 설계되었습니다.
또한, 반응형 디자인을 적용해 모바일과 데스크탑 환경 모두에서 콘텐츠가 흐트러지지 않도록 최적화하였습니다.`,
            image:
          ...
          },
          {
            ...
            contributionPercent: 100,
            contributionNote: false,
            description: `롤링페이퍼 서비스의 대표 진입점인 이 페이지는 사용자들이 인기 롤링페이퍼와 최근 작성된 롤링페이퍼를 각각의 섹션에서 쉽게 찾아볼 수 있도록 구성되었습니다. 두 섹션은 모두 카드 형태의 슬라이드 방식으로 구현되었으며, slick-carousel 라이브러리를 활용해 한 번에 하나의 카드가 자연스럽게 이동하도록 설정해 직관적인 사용자 경험을 제공합니다.
각 롤링페이퍼 카드는 썸네일 이미지, 제목, 작성자 정보를 포함하며, 클릭 시 해당 롤링페이퍼 상세 페이지로 이동하여 콘텐츠 접근성을 높였습니다.
또한, 섹션 간 시각적 구분과 인터랙션 효과를 고려하고, 전체 콘텐츠 배치와 흐름을 직접 설계해 데스크톱과 모바일 환경 모두에서 안정적인 레이아웃을 구현하였습니다.`,
            image:
          ...
          },
          {
            ...
            contributionPercent: 0,
            contributionNote: false,
            description: `사용자가 새 롤링페이퍼를 생성할 수 있는 이 페이지는 받는 사람의 이름과 배경 스타일을 선택하여 간편하게 등록할 수 있도록 구성되었습니다.
배경은 이미지 또는 컬러 중 하나를 선택할 수 있으며, 제공된 컬러 팔레트와 이미지 샘플에서 원하는 테마를 클릭해 직관적으로 선택할 수 있는 인터페이스를 제공합니다.`,
            image:
          ...
          },
          {
            ...
            contributionNote: false,
            description: `개별 롤링페이퍼의 상세 내용을 확인할 수 있는 이 페이지는 사용자 감정 표현을 위한 이모지 리액션 기능이 상단 헤더에 구현되어 있습니다. 제공되는 다양한 이모지 중 하나를 클릭하면 해당 감정에 대한 카운트가 증가하여, 소셜 피드백처럼 자연스러운 사용자 반응을 유도합니다. 또한 카카오 공유 API와 URL 복사 기능을 통해 롤링페이퍼를 손쉽게 타인에게 전달할 수 있어 콘텐츠 확산을 지원합니다.
페이지 내부에는 등록된 메시지들이 무한 스크롤 방식으로 출력되며, 각 메시지를 클릭하면 모달로 상세 내용을 확인할 수 있습니다. 개별 메시지 삭제 기능과 함께 전체 롤링페이퍼 삭제 기능도 제공되어, 사용자가 콘텐츠를 자유롭게 관리할 수 있도록 했습니다.`,
            image:
          ...
          },
          {
            ...
            contributionPercent: 0,
            contributionNote: false,
            description: `사용자가 롤링페이퍼에 메시지를 등록할 수 있는 페이지로, 이름, 프로필 이미지, 관계, 메시지, 폰트를 입력받는 폼으로 구성되어 있습니다. 프로필 이미지와 폰트는 제공된 목록에서 선택할 수 있으며, 모든 항목이 유효할 경우에만 등록 버튼이 활성화되어 사용자 실수를 방지합니다.
감성적인 시각 요소를 더한 입력 UI를 통해 직관적이면서도 개인화된 메시지를 작성할 수 있도록 설계했습니다.`,
            image:
